...
description: "Exhibition exploring grief..."
---
...
- Exhibition details (dates, location)
- Contact information
- Social media links
- Analytics settings
...
{
  "exhibition": {
    "dates": "December 19-20, 2025",  ← Change this
...
```json
{
  ...
  "href": "/submit",
  "highlight": true  ← Makes it visually distinct
}
```
...
Eric Anderson's work explores memory and material...
```
...
```json
{
  "social": {
    ...
    "instagram": "https://instagram.com/yourhandle",  ← Add URL
...
**Short copy** (headlines, navigation) → `site-metadata.json` or `navigation.json`

**Long copy** (paragraphs, articles) → `pages/*.md` files

**Structured data** (lists, menus, settings) → JSON files

**Prose content** (stories, descriptions) → Markdown files
...
2. **Preview** (once Next.js is set up, run `npm run dev`)
...
4. **Dates**: Double-check event dates and times
...
If you need to:
- **Fix a markdown formatting issue** → See markdown cheatsheet online
- **Troubleshoot JSON syntax** → Use jsonlint.com validator
- **Undo a change** → Use git to revert commit
- **Add new sections** → Follow the pattern in existing files

---

## Next Steps

Once Next.js is set up (Stage A3), these files will be automatically loaded and displayed on the website. Any changes you make will be reflected after deployment.

For now, you can edit freely to refine the copy without worrying about breaking anything.
