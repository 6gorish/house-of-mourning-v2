...
/**
 * CONSTELLATION PREVIEW - LIGHT SHADER VERSION
 * 
 * Same particle setup but with the lighter, more visible cosmic shader
 * from the shader-test page. Shows more detail in the background.
 */
...
void main() {
    ...
    // Light background - NO dimming multiplier (match shader-test exactly)
...
function ConstellationPreviewLight() {
      ...
      const sketch = (p: any) => {
        ...
        p.draw = () => {
          ...
          particles.forEach(particle => {
            ...
            gradient.addColorStop(0, `rgba(255, 220, 140, ${1.0 * brightness})`)
            gradient.addColorStop(0.4, `rgba(255, 200, 120, ${0.6 * brightness})`)
            gradient.addColorStop(0.7, `rgba(255, 200, 120, ${0.2 * brightness})`)
    ...
    <div className="fixed inset-0 w-full h-full bg-black">
      ...
      <div className="fixed top-6 right-6 font-mono text-sm text-white bg-black/80 backdrop-blur-sm px-4 py-3 rounded-lg border border-white/20 pointer-events-none z-50">
        <div className="space-y-1">
          <div className="text-lg font-bold text-blue-400">LIGHT SHADER</div>
          <div className="text-xs text-gray-400">More visible cosmic detail</div>
...
export default dynamic(() => Promise.resolve(ConstellationPreviewLight), {
  ssr: false,
})
